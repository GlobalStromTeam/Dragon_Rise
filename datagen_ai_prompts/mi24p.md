...
- `MaxHealth`：**100-300**（飞行器）
- `HudType`：`@Aircraft`
- `EngineType`：`Aircraft`；`EngineSound` 填喷气/螺旋桨音效
- `EngineInfo` 应包含：`HasGear`, `EnergyCostRate`, `Increment`, `Decrement`, `PitchSpeed`, `YawSpeed`, `RollSpeed`, `LiftSpeed`, `SpeedRate`, `GearRotateAngle`, `EngineStartSound`, `EngineSoundVolume`
- 建议补：`HasDecoy: true`（诱饵/热焰弹）、`ThirdPersonCameraPos`、`RotateOffsetHeight`
- 武器典型：`Cannon`（机炮）+ `Missile`（空空/空地）+ `Rocket` + `Bomb`（按模型 `CannonPos`/`MissilePos` 骨骼）
- 无需字段：陆地车的 `TerrainCompat` 可不填；`TrackDistanceMultiply` 不需要
...
```json
{
  "ID": "dragonrise_reforge:mi24p",
  "OBB": [
    {
      "Size": [0.848,0.575,1.602],
      "Position": [0.0,1.513,3.197]
    },
    {
      "Size": [0.848,0.388,0.82],
      "Position": [0.0,2.451,2.416]
    },
    {
      "Size": [0.848,0.106,2.133],
      "Position": [0.0,1.044,-0.397]
    },
    {
      "Size": [0.094,0.95,2.133],
      "Position": [-0.745,1.888,-0.397]
    },
    {
      "Size": [0.094,0.95,2.133],
      "Position": [0.745,1.888,-0.397]
    ...
    {
      "Size": [0.723,0.575,1.602],
      "Position": [0.0,1.951,-3.866],
      "CustomRotate": [15.0,0.0,0.0]
    },
    {
      "Size": [0.473,0.356,2.656],
      "Position": [0.0,2.507,-7.92],
      "CustomRotate": [8.75,0.0,0.0]
    },
    {
      "Size": [0.473,0.356,3.688],
      "Position": [0.0,2.945,-6.888]
    },
    {
      "Size": [1.473,0.841,2.102],
      "Position": [0.0,0.961,0.014],
      "Part": "Collision"
    },
    {
      "Size": [0.789,0.57,2.281],
      "Position": [0.0,3.202,-0.244],
      "Part": "MainEngine"
    }
  ],
  "TerrainCompat": [
  ...
  "Model": {
    "Model": "dragonrise_reforge:models/bedrock/vehicle/mi24p.geo.json",
    "Texture": "dragonrise_reforge:textures/entity/mi24p.png"
  },
  "Models": [
    {
      "Model": "dragonrise_reforge:models/bedrock/vehicle/mi24p.geo.json",
...
地形兼容骨骼: TerrainCompatPos1, TerrainCompatPos2, TerrainCompatPos3
OBB 骨骼数: 11
```
...
输出：直接给出完整的 `mi24p.json` 内容（JSON 代码块），未要求填写的字段保持原样或省略。
