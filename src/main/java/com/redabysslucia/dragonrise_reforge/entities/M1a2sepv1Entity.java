...
public class M1a2sepv1Entity extends VehicleEntity {

    // Tusk 套件安装状态（false=已拆卸/默认，true=已安装）；由左键近战点击 10 次切换
    public static final EntityDataAccessor<Boolean> TUSK_INSTALLED =
            SynchedEntityData.defineId(M1a2sepv1Entity.class, EntityDataSerializers.BOOLEAN);

    private int tuskClickCount = 0;

    public M1a2sepv1Entity(EntityType<M1a2sepv1Entity> type, Level world) {
        super(type, world);
    }
        ...
        if (!level().isClientSide()
            ...
            if (tuskClickCount >= 10) {
                ...
                player.displayClientMessage(Component.translatable(
                        installed
                                ? "message.dragonrise_reforge.m1a2sepv1.tusk_installed"
                                : "message.dragonrise_reforge.m1a2sepv1.tusk_removed"), true);
