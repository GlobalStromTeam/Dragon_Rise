...
public class AC130Model extends DragonriseVehicleModel<AC130Entity> {

    @Override
    public @Nullable TransformContext<AC130Entity> collectTransform(String boneName) {
        return switch (boneName) {
            ...
            case "propeller2" -> (bone, vehicle, state) ->
                    bone.setRotY(Mth.lerp(state.getPartialTick(), vehicle.getPropellerRotO(), vehicle.getPropellerRot()));
            case "propeller3" -> (bone, vehicle, state) ->
                    bone.setRotY(Mth.lerp(state.getPartialTick(), vehicle.getPropellerRotO(), vehicle.getPropellerRot()));
            case "propeller4" -> (bone, vehicle, state) ->
                    bone.setRotY(Mth.lerp(state.getPartialTick(), vehicle.getPropellerRotO(), vehicle.getPropellerRot()));
