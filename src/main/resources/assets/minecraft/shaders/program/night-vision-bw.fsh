...
void main() {
    ...
    if(NightVisionEnabled > 0) {
        ...
        float green = clamp(intensity / 0.59, 0.50, 1.0) * IntensityAdjust;

        vec4 visionColor = vec4(green * 0.7, green, green * 0.4, 1.0);

        float gray = dot(texColor.rgb, vec3(0.299, 0.387, 0.114));
